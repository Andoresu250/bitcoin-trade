class PurchasesController < ApplicationController
    ...
    def create
      ...
      if purchase.save
        ...
        btc = purchase.btc
        msg = "El usuario #{@user.full_name} ha realizado una compra por Ƀ#{btc}"
        sbj = "Nueva compra de #{@user.full_name}"
        User.by_admins.each {|admin| NotificationMailer.simple_notification(admin, msg, sbj).deliver }
        msg = "Hola #{@user.full_name}, gracias por confiar en nosotros has una compra de Ƀ#{btc} por valor de #{money}, validaremos tu dinero tan pronto como sea posible, y si todo esta en orden aprobaremos tu compra y se te sera transferido la cantidad de btc aquirido a tu billetera"
        sbj = "Nueva Compra"
    ...
    def deny
      if @purchase.may_deny?
        ...
        user = person.user
        money = number_to_currency(@purchase.value, unit: @purchase.country.unit)
        msg = "Hola #{user.full_name}, gracias por confiar en nosotros tu compra de Ƀ#{@purchase.btc} por valor #{money} de ha sido rechazada, hemos regresado tu saldo, lamentamos las molestias"
        sbj = "Compra fallida"
